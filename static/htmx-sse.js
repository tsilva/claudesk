...
(function() {
  ...
  htmx.defineExtension('sse', {
    ...
    init: function(apiRef) {
      ...
      // Public API: reconnect SSE on an element without full DOM reprocessing
      htmx.reconnectSSE = function(elt) {
        var url = api.getAttributeValue(elt, 'sse-connect')
        if (url) {
          ensureEventSource(elt, url, 0)
        }
      }
    },
  ...
  function ensureEventSource(elt, url, retryCount) {
    ...
    // If an EventSource already exists for this exact URL, no-op
    if (internalData.sseEventSource && internalData.sseEventSourceUrl === url) {
      return
    }

    // If URL changed, close the old EventSource before creating a new one
    ...
    source.onopen = function(evt) {
      ...
      // Always re-register child sse-swap elements so they bind to the new EventSource
    ...
    internalData.sseEventSourceUrl = url
